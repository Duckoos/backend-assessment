...
export default function DashboardPage() {
  ...
  const handleCreateTask = async (e) => {
    ...
    try {
      await api.post('/tasks', { title: newTaskTitle, status: 'pending', priority: 'medium' });
      setNewTaskTitle('');
      fetchTasks();
    } catch (error) {
      ...
      <div className="px-4 py-6 sm:px-0">
        ...
        <form onSubmit={handleCreateTask} className="mb-8 flex gap-4">
            <input 
                ...
                className="flex-1 rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm border p-3"
            />
            <button 
                type="submit"
